...
### ==================================================================== ###
#
# $Id: mdmsconfig.sh,v 1.20 2017/12/21 23:31:56 awt Exp $
#
...
checkVersions()
{
  ...
  P1=`echo $ARG1 | sed -n 's/.*\(\.\).*/\1/p'`
  P2=`echo $ARG2 | sed -n 's/.*\(\.\).*/\1/p'`
  if [ ! "$P1" = "." ]
    then ARG1=$ARG1".0"
  fi
  if [ ! "$P2" = "." ]
...
#    fi
    ...
    GCRV_VERSION=`echo ${GCRV_VERSIONSTR} | sed -n 's/^[^0-9]*\([0-9][0-9]*\.[0-9][0-9]*\).*$/\1/p'`
...
#            NOTE 1: V2HTML logic is ignored
...
getClasspath() {
    ...
    # removed checking V2HTML variable
...
getJVMArgs() {
    ...
    GJA_ARGS="${GJA_ARGS} -Djavax.net.ssl.trustStorePassword=imfine"
    GJA_ARGS="${GJA_ARGS} -Djavax.net.ssl.trustStoreType=PKCS12"
