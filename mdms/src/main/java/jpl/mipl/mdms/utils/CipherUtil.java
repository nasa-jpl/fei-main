...
public class CipherUtil {
    ...
    // SunJCE transformation name (standard JCE / non-FIPS environments)
    private static final String CIPHER_TRANSFORMATION = "RSA/ECB/OAEPWithSHA-256AndMGF1Padding";
    // BC-FIPS transformation name (section 4.2 of BC-FJA User Guide); used explicitly
    // to bypass SunPKCS11 which does not support mode NONE.
    private static final String CIPHER_TRANSFORMATION_BCFIPS = "RSA/NONE/OAEPwithSHA256andMGF1Padding";
...
//        }
    ...
    /**
     * Get a Cipher instance for RSA/OAEP. When the BC-FIPS provider is registered,
     * it is used explicitly to avoid SunPKCS11 intercepting the request (PKCS11
     * does not support mode NONE, and its OAEP defaults can diverge from BC-FIPS).
     * Falls back to the standard JCE provider lookup when BC-FIPS is not present
     * (non-FIPS environments).
     *
     * <p>NOTE: This method does NOT register BC-FIPS itself. The caller must ensure
     * that BC-FIPS has been registered (e.g. via SecureSocketsUtil) before
     * constructing a CipherUtil instance on a FIPS-enabled system.
     *
     * @return Cipher instance for RSA OAEP with SHA-256
     * @throws Exception if no provider supports the transformation
     */
    private Cipher getRsaOaepCipher() throws Exception {
        Provider bcFips = Security.getProvider(BCFIPS_PROVIDER_NAME);
        if (bcFips != null) {
            return Cipher.getInstance(CIPHER_TRANSFORMATION_BCFIPS, bcFips);
        }
    ...
    private void readPrivateKey() throws IOException {
        ...
        try {
            ...
            this._dcipher = getRsaOaepCipher();
            this._dcipher.init(Cipher.DECRYPT_MODE, this._privateKey, OAEP_PARAMS);

        } catch (Exception exc) {
            this._logger.error(exc.getMessage(), exc);
            throw new IOException("Failed to initialize decryption cipher: " + exc.getMessage(), exc);
        }
    ...
    private void readPublicKey() throws IOException {
        ...
        try {
            ...
            this._cipher = getRsaOaepCipher();

            this._cipher.init(Cipher.ENCRYPT_MODE, this._publicKey, OAEP_PARAMS);

        } catch (Exception exc) {
            this._logger.error(exc.getMessage(), exc);
            throw new IOException("Failed to initialize encryption cipher: " + exc.getMessage(), exc);
        }
