 ...
 * controlled by a control string. <p/>A control string is a Java string that
 * contains a control specification. The control specification starts at the
 ...
 * </p>
 * <p/>The format is composed of zero or more directives defined as follows:
 * <ul>
 ...
 * </ul>
 * </p>
 * <p/>The results are undefined if there are insufficient arguments for the
 ...
 * otherwise ignored. In format strings containing the % form of conversion
 * specifications, each argument in the argument list is used exactly once.
 * </p>
 * <p/>Conversions can be applied to the <code>n</code> th argument after the
 ...
 * </p>
 * <p/>In format strings containing the %<code>n</code>$ form of conversion
 * specifications, each argument in the argument list is used exactly once.
 * </p>
 * <p/>
 * <h4>Escape Sequences</h4>
 * <p/>The following table lists escape sequences and associated actions on
 * display devices capable of the action. <table>
 * <tr>
 ...
 * <p/>Each conversion specification is introduced by the percent sign character
 * (%). After the character %, the following appear in sequence:
 * </p>
 * <p/>Zero or more flags (in any order), which modify the meaning of the
 * conversion specification.
 * </p>
 * <p/>An optional minimum field width. If the converted value has fewer
 * characters than the field width, it will be padded with spaces by default on
 * the left; t will be padded on the right, if the left- adjustment flag (-),
 * described below, is given to the field width. The field width takes the form
 * of a decimal integer. If the conversion character is s, the field width is
 * the the minimum number of characters to be printed.
 * </p>
 * <p/>An optional precision that gives the minumum number of digits to appear
 * for the d, i, o, x or X conversions (the field is padded with leading zeros);
 ...
 * </p>
 * <p/>An optional h specifies that a following d, i, o, x, or X conversion
 ...
 * before printing).
 * </p>
 * <p/>An optional l (ell) specifies that a following d, i, o, x, or X
 * conversion character applies to a type long argument.
 * </p>
 * <p/>A field width or precision may be indicated by an asterisk (*) instead of
 * a digit string. In this case, an integer argument supplised the field width
 ...
 * </p>
 * <p/>In format strings containing the %<code>n</code>$ form of a
 * conversion specification, a field width or precision may be indicated by the
 * sequence *<code>m</code>$, where m is a decimal integer giving the
 * position in the argument list (after the format argument) of an integer
 * argument containing the field width or precision.
 * </p>
 * <p/>The format can contain either numbered argument specifications (that is, %
 ...
 * results of mixing numbered and unnumbered argument specifications in a format
 * string are undefined.
 * </p>
 * <p/>
 * <h4>Flag Characters</h4>
 * <p/>The flags and their meanings are:
 * </p>
 * <dl>
 * <dt>'
 ...
 * <p/>Each conversion character results in fetching zero or more arguments. The
 ...
 * </p>
 * <p/><p/>The conversion characters and their meanings are:
 * </p>
 * <dl>
 * <dt>d,i
 ...
 * </dl>
 * <p/>If a conversion specification does not match one of the above forms, an
 * IllegalArgumentException is thrown and the instance of PrintfFormat is not
 * created.
 * </p>
 * <p/>If a floating point value is the internal representation for infinity,
 * the output is [+]Infinity, where Infinity is either Infinity or Inf,
 * depending on the desired output string length. Printing of the sign follows
 * the rules described above.
 * </p>
 * <p/>If a floating point value is the internal representation for
 * "not-a-number," the output is [+]NaN. Printing of the sign follows the rules
 * described above.
 * </p>
 * <p/>In no case does a non-existent or small field width cause truncation of a
 ...
 * </p>
 * <p/>The behavior is like printf. One exception is that the minimum number of
 ...
 * </p>
 * <p/>The biggest divergence from the C printf specification is in the use of
 * 16 bit characters. This allows the handling of characters beyond the small
 * ASCII character set and allows the utility to interoperate correctly with the
 * rest of the Java runtime environment.
 * </p>
 * <p/>Omissions from the C printf specification are numerous. All the known
 ...
 * characters only.
 * </ul>
 * <p/>Most of this specification is quoted from the Unix man page for the
 * sprintf utility.
 * </p>
 *
 * @author Allan Jacobs
 ...
 *          50000...
 * @see http://java.sun.com/developer/technicalArticles/Programming/sprintf/PrintfFormat.java
 */
public class PrintfFormat {
    ...
    * @throws IllegalArgumentException if the control string is null, zero
    ...
    * @return The formatted String.
    * @throws IllegalArgumentException if the conversion character is f, e, E,
    *            g, G, s, or S.
    */
    ...
    * @return The formatted String.
    * @throws IllegalArgumentException if the conversion character is c, C, s,
    *            S, d, d, x, X, or o.
    */
    ...
    * @return The formatted String.
    * @throws IllegalArgumentException if the conversion character is neither s
    *            nor S.
    */
    ...
    * internal formatting method for Strings. Otherwise use the default
    * formatter (use toString).
    *
    * @param x the Object to format.
    * @return the formatted String.
    * @throws IllegalArgumentException if the conversion character is
   ...
   /**
    * <p/>ConversionSpecification allows the formatting of a single primitive or
    ...
    * <p/>A format string is a Java string that contains a control string. The
    ...
    * string.
    * </ol>
    * <p/>A control string takes the form:
    *
    * <pre>
    *
    *   % ['-+ #0]* [0..9]* { . [0..9]* }+
    ...
    * </pre>
    *
    * <p/>The behavior is like printf. One (hopefully the only) exception is
    * that the minimum number of exponent digits is 3 instead of 2 for e and E
   ...
   private class ConversionSpecification {
       ...
       * @throws IllegalArgumentException if the input string is null, zero
       *            length, or otherwise malformed.
       */
       ...
       * @return the formatted String.
       * @throws IllegalArgumentException if the conversion character is f, e,
       *            E, g, or G.
       */
       ...
       * @return the formatted String.
       * @throws IllegalArgumentException if the conversion character is c, C,
       *            s, S, i, d, x, X, or o.
       */
       ...
       * @return the formatted String.
       * @throws IllegalArgumentException if the conversion character is neither
       *            s nor S.
       */
       ...
       * implies that padding to the field width will be done with zeros instead
       * of blanks. <p/>The field width is treated as the minimum number of
       * characters to be printed. The default is to add no padding. Padding is
       * with blanks by default. <p/>The precision, if set, is the number of
       * digits to appear after the radix character. Padding is with trailing
       * 0s.
       */
       ...
       * of blanks. <p/>The field width is treated as the minimum number of
       * characters to be printed. The default is to add no padding. Padding is
       * with blanks by default. <p/>The precision, if set, is the minimum
       * number of digits to appear after the radix character. Padding is with
       * trailing 0s. <p/>The behavior is like printf. One (hopefully the only)
       * exception is that the minimum number of exponent digits is 3 instead of
       * 2 for e and E formats when the optional L is used before the e, E, g,
       * or G conversion character. The optional L does not imply conversion to
       * a long long double.
       */
       ...
       * @param icarry the index of the first digit that is to be truncated from
       *           the print
      ...
      private boolean checkForCarry(char[] ca1, int icarry) {
         ...
         if (icarry < ca1.length) {
            if (ca1[icarry] == '6' || ca1[icarry] == '7' || ca1[icarry] == '8'
                  || ca1[icarry] == '9')
               carry = true;
      ...
      /**
       * Format method for the g conversion character. <p/>For g format, the
       * flag character '-', means that the output should be left justified
       ...
       * -). The blank flag character means that a non-negative input will be
       * preceded with a blank. If both a '+' and a ' ' are specified, the blank
       * flag is ignored. The '0' flag character implies that padding to the
       * field width will be done with zeros instead of blanks. <p/>The field
       * width is treated as the minimum number of characters to be printed. The
       * default is to add no padding. Padding is with blanks by default. <p/>
      ...
      /**
       * Format method for the d conversion specifer and short argument. <p/>
       ...
       * characters to be printed. The default is to add no padding. Padding is
       * with blanks by default. <p/>The precision, if set, is the minimum
       * number of digits to appear. Padding is with leading 0s.
       *
      ...
      /**
       * Format method for the d conversion character and long argument. <p/>
       * For d format, the flag character '-', means that the output should be
       * left justified within the field. The default is to pad with blanks on
       * the left. A '+' character means that the conversion will always begin
       * with a sign (+ or -). The blank flag character means that a
      ...
      /**
       * Format method for the d conversion character and int argument. <p/>For
       * d format, the flag character '-', means that the output should be left
       * justified within the field. The default is to pad with blanks on the
       * left. A '+' character means that the conversion will always begin with
       * a sign (+ or -). The blank flag character means that a non-negative
       * input will be preceded with a blank. If both a '+' and a ' ' are
       * specified, the blank flag is ignored. The '0' flag character implies
       * that padding to the field width will be done with zeros instead of
       * blanks. <p/>The field width is treated as the minimum number of
       * characters to be printed. The default is to add no padding. Padding is
       * with blanks by default. <p/>The precision, if set, is the minimum
       * number of digits to appear. Padding is with leading 0s.
       *
      ...
      /**
       * Format method for the x conversion character and short argument. <p/>
       * For x format, the flag character '-', means that the output should be
       * left justified within the field. The default is to pad with blanks on
       * the left. The '#' flag character means to lead with '0x'. <p/>The field
       * width is treated as the minimum number of characters to be printed. The
       * default is to add no padding. Padding is with blanks by default.
       * <p/>The precision, if set, is the minimum number of digits to appear.
      ...
      /**
       * Format method for the x conversion character and long argument. <p/>
       * For x format, the flag character '-', means that the output should be
       * left justified within the field. The default is to pad with blanks on
       * the left. The '#' flag character means to lead with '0x'. <p/>The field
       * width is treated as the minimum number of characters to be printed. The
      ...
      /**
       * Format method for the x conversion character and int argument. <p/>For
       * x format, the flag character '-', means that the output should be left
       * justified within the field. The default is to pad with blanks on the
       * left. The '#' flag character means to lead with '0x'. <p/>The field
       * width is treated as the minimum number of characters to be printed. The
       * default is to add no padding. Padding is with blanks by default. <p/>
      ...
      /**
       * Format method for the o conversion character and short argument. <p/>
       ...
       * leading 0 and the precision is increased by 1. <p/>The field width is
       * treated as the minimum number of characters to be printed. The default
       * is to add no padding. Padding is with blanks by default. <p/>The
       * precision, if set, is the minimum number of digits to appear. Padding
       * is with leading 0s.
       *
       * @param x the short to format.
       * @return the formatted String.
      ...
      /**
       * Format method for the o conversion character and long argument. <p/>
       * For o format, the flag character '-', means that the output should be
       * left justified within the field. The default is to pad with blanks on
       * the left. The '#' flag character means that the output begins with a
       * leading 0 and the precision is increased by 1. <p/>The field width is
      ...
      /**
       * Format method for the o conversion character and int argument. <p/>For
       * o format, the flag character '-', means that the output should be left
       * justified within the field. The default is to pad with blanks on the
      ...
      /**
       * Format method for the c conversion character and char argument. <p/>
       ...
       * pad with blanks on the left. <p/>The field width is treated as the
       * minimum number of characters to be printed. Padding is with blanks by
       * default. The default width is 1. <p/>The precision, if set, is ignored.
       *
       * Update: 2018-12-03: William: fixing security scan - Update the logic to remove the character array
       *
       * @param x the char to format.
       * @return the formatted String.
       */
      private String printCFormat(char x) {
         if (!fieldWidthSet || fieldWidth < 2) { // field width is not set. converting char to string
            return String.valueOf(x);
         }
         char paddingChar = ' ';
         StringBuilder pads = new StringBuilder();
         for (int i = 0; i < fieldWidth - 1; i++) {
            pads.append(paddingChar);
         }
         if (leftJustify) { // left justified. character and pads
            return x + pads.toString();
         }
         return pads.toString() + x; // right justified. pads and character.
      }

      /**
       * Format method for the s conversion character and String argument. <p/>
       * The only flag character that affects s format is the '-', meaning that
       * the output should be left justified within the field. The default is to
       * pad with blanks on the left. <p/>The field width is treated as the
       * minimum number of characters to be printed. The default is the smaller
       * of the number of characters in the the input and the precision. Padding
       * is with blanks by default. <p/>The precision, if set, specifies the
       * maximum number of characters to be printed from the string. A null
       * digit string is treated as a 0. The default is not to set a maximum
       ...
       * formats. An l or h is used to control conversion of the input to a long
       * or short, respectively, before formatting. If any of these is present,
       * store them.
       */
      ...
      private boolean setFieldWidthArgPosition() {
         ...
         if (xPos > pos && xPos < fmt.length()) {
            if (fmt.charAt(xPos) == '$') {
               ...
               argumentPositionForFieldWidth = Integer.parseInt(fmt.substring(
                     pos, xPos));
               pos = xPos + 1;
               ret = true;
            }
       ...
       * the result to be a zero. For x (or X) conversions, a non-zero result
       * will have 0x (or 0X) prepended to it. For e, E, f, g, or G conversions,
       * the result will always contain a radix character, even if no digits
       * follow the point. For g and G conversions, trailing zeros will not be
