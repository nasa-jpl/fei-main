...
public class BufferedStreamIO
{
      ...
      try {
         ...
         synchronized (BufferedStreamIO._LOCK)
         {
            if (BufferedStreamIO._digest == null)
               BufferedStreamIO._digest = MessageDigest.getInstance("SHA-1");
         }
    ...
    * recommended.
    *
    * Update: 2018-12-03: William: fixing security scan - convert long to int before comparing
    *
    * @param buffer data buffer to store data read from input stream.
    * @param fileSize file size in unit of bytes
                                                                     ...
                                                                     throws IOException
   {
      ...
      if (buffer.length < fileSize) { // this validation also ensures that fileSize < Integer.MAX_VALUE
         throw new IOException("Insufficient buffer size for the incoming file size.");
      }

      int fileSizeInt = (int) fileSize; // can do this as the above validation ensures that there is no overflow.
      int retVal;
      int bytesRead = 0;

      while (bytesRead < fileSizeInt)
      {
         retVal = this._bis.read(buffer, bytesRead, fileSizeInt - bytesRead);
         if (retVal < 1)
            throw new IOException("Unexpected EOF from network peer.");
         bytesRead += retVal;
      }
                                         ...
                                         throws IOException, VerifyException
   {
      ...
      try {
         ...
         while (bytesRead < fileSizeInt)
         {
            retVal = this._digestis.read(buffer, bytesRead, fileSizeInt - bytesRead);
            if (retVal < 1)
               throw new IOException("Unexpected EOF network peer.");
            bytesRead += retVal;
         }
