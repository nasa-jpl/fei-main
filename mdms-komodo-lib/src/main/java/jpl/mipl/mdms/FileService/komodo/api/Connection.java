   ...
   private void _makeConnection() throws IOException, SecurityException {
      try {
         ...
         case Constants.SSL:
            ...
            // Explicitly start TLS handshake before wrapping streams.
            // Without this, the handshake is lazy (deferred to first read/write).
            // When login=false connections start multiple service threads (proxy,
            // control, pulse) simultaneously, those threads race to trigger the
            // handshake concurrently, corrupting the TLS state and causing
            // SSLException on both client and server.  Completing the handshake
            // here (in the constructor thread, before any service threads start)
            // guarantees a clean, sequential handshake.
            this._sslClient.startHandshake();
            this._logger.trace("TLS handshake complete.");
            /*
